...
export const GET = async (request: Request) => {
  ...
  try {
    ...
    if (!savedByUser)
      return NextResponse.json("ERRORS.NO_USER_API", { status: 200 });

    const mappedMindMaps: StarredItem[] = savedByUser.savedMindMaps.map(
      (mindMap) => ({
        id: mindMap.id,
    ...
    const sortedSavedInfo = mappedSavedInfo.sort((a, b) =>
      compareDates(a, b, sortType)
    );
